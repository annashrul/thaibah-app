class ApiService {
 // ################################# START PRODUCTION ############################//
 // final bool showCode      = false;
 // final String versionCode = '1.3.6';
 // final String baseUrl     = "http://thaibah.com:3000/api/v1/";
 // ################################# END PRODUCTION ############################//

 // ################################# START DEVELOPMENT ############################//
 final bool showCode       = true;
 final String versionCode  = '1.2.4';
 final String baseUrl      = "http://thaibah.com:3010/api/v1/";
 // ################################# END DEVELOPMENT ############################//

 final String assetsLocal  = "assets/images/";
 final String assetsServer = "https://thaibah.com/assets/icon/baru/";
 final String baseAssetsRevisi   = "https://thaibah.com/assets/icon/";
 final String baseAssets   = "http://thaibah.com:3000/assets/";
 final String noImage      = 'https://icon-library.com/images/no-image-icon/no-image-icon-13.jpg';
 final String token        = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiJiZWEyNDQ0ZC0zZDZiLTQyNGItYTQ3YS00OTBiY2I3NTYwYjgiLCJpYXQiOjE1NzEwNDI0OTEsImV4cCI6MTU3MzYzNDQ5MX0.lvRR9rU3cdLbJ9Gi0Ryucwl_7IMK6NF7BTblIQUDA9Q";
 final String iconUrl      = 'https://thaibah.com/assets/icon/';
 final String username     = 'netindo';
 final String password     = 'admin1234!@';
 final int timerActivity   = 120;
 final bool isActivity     = false;
 final String deviceId     = '6a4c55fd-d96d-427f-8634-d2c4b9d96d69';
 final String tokenExpiredError     = 'TokenExpiredError';
 final String timeoutException     = 'TimeoutException';
 final String socketException     = 'SocketException';

}

