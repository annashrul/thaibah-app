...
class AddressProvider {
  ...
  Future<AddressModel> fetchAlamat() async{
    ...
    final response = await client.get(ApiService().baseUrl+'member/addr/list',headers: {'Authorization':token});
  ...
  Future<GetAddressModel> fetchGetAddress(var id) async{
    final token = await userRepository.getToken();
    final response = await client.get(ApiService().baseUrl+'id',headers: {'Authorization':token});
    if (response.statusCode == 200) {
  ...
  Future fetchDeleteAddress(var id) async {
    final token = await userRepository.getToken();
    return await client.post(ApiService().baseUrl+"member/addr/delete",headers: {'Authorization':token},
        body: {
          "id":"$id",
  ...
  Future fetchUpdateMyBank(var title,var name,var main_address,var kd_prov,var kd_kota, var kd_kec, var no_hp, var id) async {
    final token = await userRepository.getToken();
    return await client.post(ApiService().baseUrl+"member/addr/update",headers: {'Authorization':token},
        body: {
          "title":"$title",
          "name":"$name",
  ...
  Future fetchCreateAddress(var title,var name,var main_address,var kd_prov,var kd_kota, var kd_kec, var no_hp) async {
    final token = await userRepository.getToken();
    return await client.post(ApiService().baseUrl+"member/addr/create",headers: {'Authorization':token},
        body: {
          "title":"$title",
          "name":"$name",
          "main_address":"$main_address",
